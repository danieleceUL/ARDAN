function nssfr_analysis(selpath, tt_img, dataH, dataV)
    ...
    meanMTF50hName = 'mean_horizontal_MTFs_per_Annuli';
    meanMTF50vName = 'mean_vertical_MTFs_per_Annuli';
    hMpMTF50hName = 'surface_plot_horizontal_MTF50_mean';
    hMpMTF50vName = 'surface_plot_vertical_MTF50_mean';
    spa_dist_hmp_loc = 'spatial_dist_heatmap_locations';
    spa_dist_HROI = 'spatial_dist_horizontal_ROIs';
    spa_dist_VROI = 'spatial_dist_vertical_ROIs';
    cust_seg = 'custom radial segments';
    ...
    spatial = {spa_dist_hmp_loc, spa_dist_HROI, spa_dist_VROI, cust_seg};
    for iFig = 1:length(FigList)
        FigHandle = FigList(iFig);
        FigName   = get(FigHandle, 'Name');
        set(0, 'CurrentFigure', FigHandle);
        cls = class(gca);
        if ~isequal(cls, 'matlab.graphics.chart.HeatmapChart') && contains(FigName, spatial)
            axis image
            xlim([0 size(tt_img, 2)])
            ylim([0 size(tt_img, 1)])

        end
        if contains(FigName, {meanMTF50hName, meanMTF50vName})
            mtf50data = PlotMTF50(FigName);
            ...
            %Write CSV data
            fid = fopen([selpath filesep FigName '.csv'], 'w');
        ...
        if isequal(FigName, spa_dist_hmp_loc)
            f = gcf;
            ygridvals = f.Children.YTick;
            xgridvals = f.Children.XTick;
            centroidh = [dataH(:,10), dataH(:,11)];
            centroidv = [dataV(:,10), dataV(:,11)];
            countHGrid = countPoints(centroidh, xgridvals, ygridvals);
            countVGrid = countPoints(centroidv, xgridvals, ygridvals);
            format long
            HNumPointsPerGrid = 'horizontal_points_per_grid';
            VNumPointsPerGrid = 'vertical_points_per_grid';
            figure('Name', HNumPointsPerGrid)
            hmpHct = heatmap(countHGrid)
            figure('Name', VNumPointsPerGrid)
            hmpVct = heatmap(countVGrid)
            saveas(hmpHct, [selpath filesep HNumPointsPerGrid], 'fig')
            saveas(hmpHct, [selpath filesep HNumPointsPerGrid], 'png')
            saveas(hmpVct, [selpath filesep VNumPointsPerGrid], 'fig')
            saveas(hmpVct, [selpath filesep VNumPointsPerGrid], 'png')
