...
[selmsk, path] = uigetfile('./masks/*.mat', 'Select custom ROI mask')
            ...
            if ~isempty(r{a,1})
                ...
                if ~isempty(dat)
                    if ~isnan(dat(:,end))
                        ...
                        emp2=isempty(BadMTF);
                        if (emp2==1 || size(BadMTF,1)<=5) && (bad_mtf_bd == 0) % 5=10%
                            Dat = [uq,mq];
                            R{a,1}=Dat;
                            if debug
...
toc
disp('Completed NS-SFR Extraction');
