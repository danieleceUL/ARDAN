...
[Rad, RD] = RadAnnuli_custom(RDseg, ROImask, tt_img);

%divide ROImask up into grid segments
rec_x = size(ROImask,2)/nx;
rec_y = size(ROImask,1)/ny;
...
hold on
grid on
xticks(0:rec_x:size(ROImask, 2))
yticks(0:rec_y:size(ROImask, 1))
...
hold on
grid on
xrange = 0:rec_x:size(ROImask, 2);
yrange = 0:rec_y:size(ROImask, 1);
