...
% ROIisolate is function to isolate the crops using the edge isolation
% binary image. Binary Boxing is used to for this purpose. 
% 
% Input:
%       Im:     Orginal (linear) image
...
% figure, imshow(Im) %DJ - printing using red rectangles ROIs
% hold on
% for k = 1 : length(bb)
...
% % % %             masks=regionprops(mask,'PixelIdxList');
        ...
        % Insert the new ROI into BB cell array, removing the large ROI
        % Insert using insertrows (Copyright (c) 2016, Jos van der Geest)
...
% % % %         end
        ...
        if abs(stepedgetotal)>=SEnum % yes = step-edge
            ...
            %--------------------------------------------------------------
            % Edge Mask
            %DJ - something wrong with edge mask - returning 0s
            mask=imcrop(Ma, thisBB);
            checkMask = all(mask(:,:) == 0);
            check = all(checkMask(:) == 1);
            if check ~= 1
                % Remove any 'non-main' edges in ROI 
                l = bwlabel(mask);
                ...
                % Sort based on number of pixels in each connected component:
                d = [masks.Area]; %max number of diagonal pixels in each region
                [~,order] = sort(d,'descend');
%                 order
%                 l
%                 d
%                 thisBB
                ...
                ROIs{K,6}= [thisBB(1) + thisBB(3)/2  thisBB(2) + thisBB(4)/2];
            end
        else
            continue
        end
