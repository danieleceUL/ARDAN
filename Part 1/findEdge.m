function [BWh, BWv, Mh, Mv, SEh, SEv]=findEdge(img, maskedImagePerim, ST, esfW, MinEdge)
    % findEdge is an adaption of the canny edge detector, providing vertical
    % and horizontal edges separately, edge masks and determines step-edges.
    %
    % Input:
    %    img:       The Image.
    ...
    %
    % Adapted from MATLAB function [BW,thresh] = EDGE(I,'canny',...)
    ...
    % Re-Apply: Remove all edges smaller than 20 pixels in length:
    bwh= EdgeReduction(bwh, MinEdge);
    bwv= EdgeReduction(bwv, MinEdge);
    %--------------------------------------------------------------------------
    ...
    % Threshold the Gradient Array - anything between -Var2 and Var2 = 0,
    % else if positve = 1 or negative =-1
    SE = strel('square',2);
    SE2 = strel('square',7);
    SE3 = strel('square',10);
    %dilate the mask boundary to remove boundary boxes
    maskedImagePerim_dil = imdilate(maskedImagePerim, SE3);
    %imshow(maskedImagePerim_dil);
    for O=1:2
        switch O
            case 1 % H
...
%         if isequal(bw, bwh)
%             BW = logical(BW) - rot90(maskedImagePerim_dil);
%         else
%             BW = logical(BW) - maskedImagePerim_dil;
%         end
        ...
        %DJ - added in Mask subtraction to eliminate boundary ROIs
        if isequal(bw, bwh)
            Mask = Mask - rot90(maskedImagePerim_dil);
        else
            Mask = Mask - maskedImagePerim_dil;
        end
        %maskedImagePerim_dil_c = imdilate(maskedImagePerim, SE2);
    ...
    %         y(:)=Y(yy);
            ...
            for ii = 1:max(labeledMask)
                ...
                % Is Step Edge? (measures the gradient of a profile to determine
                % if there is a single positive or negarive gradient,
                % otherwise stepedge=0)
                % DJ problem with stepedge should be done after BWh and BWv are
                % assigned
                gpos = 0;
                gneg = 0;
                ...
                %placed it into function
                %stepedge = make_stepedge(X, Y, xx, yy);
    ...
    %         Row=MaskGrad(Y(yy),:);
        ...
        case 1 % H
            BWh = logical(BW);
            Mh =  Mask;
            Mh = Mh - rot90(maskedImagePerim_dil);
            SEh = stepedge;
        case 2 % V
            BWv = logical(BW);
            Mv =  Mask; 
            Mv = Mv - maskedImagePerim_dil;
            SEv = stepedge;
        end
    end
end
...
function SE = make_stepedge(X, Y, xx, yy)
